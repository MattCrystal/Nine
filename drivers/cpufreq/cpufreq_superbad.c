...
static void cpufreq_superbad_timer(unsigned long data)
{
	...
	unsigned long new_rate;
        ...
        delta_idle = (now_idle - this_superbad->time_in_idle);
        delta_time = (update_time - this_superbad->idle_exit_time);
        //printk(KERN_INFO "superbadT: t=%llu i=%llu\n",(update_time - this_superbad->idle_exit_time),delta_idle);
        ...
        if ((cpu_load > max_cpu_load || delta_idle == 0) &&
            !(policy->cur > this_superbad->max_speed &&
              (update_time - this_superbad->freq_change_time) > 100*down_rate_us)) {
		...
		if (policy->cur > up_min_freq) new_rate = 75000;
                ...
                if ((update_time - this_superbad->freq_change_time) < new_rate) 
                        return;
         ...
         * minimum sample time.
         */
        if ((update_time - this_superbad->freq_change_time) < down_rate_us)
                return;
